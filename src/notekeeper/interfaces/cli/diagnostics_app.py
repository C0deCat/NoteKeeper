...
def register_command(app: typer.Typer, runtime_factory: RuntimeFactory) -> None:
    ...
    def diagnostics(campaign_id: str | None = None) -> None:
        ...
        typer.echo(f"whisperx_language={snapshot.whisperx_language or 'auto'}")
        ...
        typer.echo(f"deepseek_model={snapshot.deepseek_model_name}")
        typer.echo(f"deepseek_temperature={snapshot.deepseek_temperature:.1f}")
